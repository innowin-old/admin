<template>
  <div>
    <v-container>
      <v-card-title>
        <div class="buttons">
          <v-btn color="success" dark v-on:click="navigate('Import-Products')">Import</v-btn>
          <v-btn color="pink" dark>Export</v-btn>
          <v-btn color="cyan" dark v-on:click="navigate('/products/form')">New Product</v-btn>
        </div>
        <v-spacer></v-spacer>
        <v-text-field
          append-icon="search"
      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td>
            <v-btn flat icon color="orange" class="tools-button" v-on:click="navigate('/products/form?id=' + props.item._id)">
              <v-icon>edit</v-icon>
            </v-btn>
    ...
    data: function () {
      return {
        ...
        headers: [
          ...
          { text: 'Category', value: 'product_category', align: 'left' },
