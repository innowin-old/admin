<template>
    ...
    <div>
        <v-container>
            <v-card-title>
                <div class="buttons">
                    <v-btn color="success" dark v-on:click="navigate('Import-Provinces')">Import</v-btn>
                    <v-btn color="pink" dark>Export</v-btn>
                    <v-btn color="cyan" dark v-on:click="navigate('/gis/provinces/form')">New Province</v-btn>
                </div>
                <v-spacer></v-spacer>
                <v-text-field
                        append-icon="search"
    ...
    export default {
        ...
        methods: {
            ...
            deleteRecord: function (id, index) {
                this.$swal({
                    ...
                    if (result.value) {
                        this.$store.commit('provinces/remove', id)
                        this.$swal(
                            'خذف شد!',
        ...
        computed: {
            pages () {
                ...
                return Math.ceil(this.province_states.length / this.pagination.rowsPerPage)
            }
        },
        mounted: function () {
