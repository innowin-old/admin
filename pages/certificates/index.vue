    ...
    <v-container>
      <v-card-title>
        <div class="buttons">
          ...
          <v-btn color="cyan" dark v-on:click="navigate('/certificates/form')">New Certificate</v-btn>
        </div>
        <v-spacer></v-spacer>
        <v-text-field
          append-icon="search"
      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td>
            <v-btn flat icon color="orange" class="tools-button" v-on:click="navigate('/certificates/form?id=' + props.item._id)">
              <v-icon>edit</v-icon>
            </v-btn>
    ...
    methods: {
      ...
      deleteRecord: function (id, index) {
        this.$swal({
          ...
          if (result.value) {
            this.$store.commit('certificates/remove', id)
