<template>
  <div>
    <v-container>
      <v-card-title>
        <div class="buttons">
          <v-btn color="success" dark v-on:click="navigate('Import-Urls')">Import</v-btn>
          <v-btn color="pink" dark>Export</v-btn>
          <v-btn color="cyan" dark v-on:click="navigate('/users/urls/form')">New Url</v-btn>
        </div>
        <v-spacer></v-spacer>
        <v-text-field
          append-icon="search"
      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td>{{ props.item.identity_url_related_identity }}</td>
          <td>
            <v-btn flat icon color="orange" class="tools-button" v-on:click="navigate('/users/urls/form?id=' + props.item._id)">
              <v-icon>edit</v-icon>
            </v-btn>
    ...
    data: function () {
      return {
        ...
        headers: [
          ...
          { text: 'Related Identity', value: 'identity_url_related_identity', align: 'left' },
          { value: '' }
        ],
        dialog: false,
        notifications: false,
    ...
    methods: {
      ...
      deleteRecord: function (id, index) {
        this.$swal({
          ...
          if (result.value) {
            this.$store.commit('urls/remove', id)
