<template>
  <div>
    <v-container>
      <v-card-title>
        <div class="buttons">
          <v-btn color="success" dark v-on:click="navigate('Import-Researches')">Import</v-btn>
          <v-btn color="pink" dark>Export</v-btn>
          <v-btn color="cyan" dark v-on:click="navigate('/users/researches/form')">New Research</v-btn>
        </div>
        <v-spacer></v-spacer>
        <v-text-field
          append-icon="search"
      ...
      >
        <template slot="items" slot-scope="props">
          ...
          <td>
            <v-btn flat icon color="orange" class="tools-button" v-on:click="navigate('/users/researches/form?id=' + props.item._id)">
              <v-icon>edit</v-icon>
            </v-btn>
    ...
    data: function () {
      return {
        ...
        headers: [
          ...
          { text: 'Url', value: 'link', align: 'left' },
          { text: 'Author', value: 'author', align: 'left' },
          { text: 'Publication', value: 'publication', align: 'left' },
          { value: '' }
        ],
        dialog: false,
        notifications: false,
    ...
    computed: {
      ...
      items: function () {
        if (typeof this.$store !== 'undefined') {
          return this.$store.state.researches.list
        } else {
          return []
        }
      }
    ...
    methods: {
      ...
      deleteRecord: function (id, index) {
        this.$swal({
          ...
          if (result.value) {
            this.$store.commit('researches/remove', id)
